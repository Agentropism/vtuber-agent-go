...
| --- | --- |
| 实施顺序 | 五项一次全做 |
| 推流启停 | 改成可启停生命周期对象（`Start`/`Stop`/`Running`），API 控制 |
| 模型切换 | 热切换：重建 catalog 与表情词表 + 给已连前端补发 hello |
| TTS 试听 | 合成后直接返回 WAV，不入播报队列 |
| 记忆删除 | 墓碑标记，读取时过滤（文件只增不重写） |
| 工具边界 | 只读工具可直接调用，有副作用的 403 |

执行：`7dc4859`（记忆 + 工具）、`2fd29b2`（模型 + TTS + 推流），每步 `gofmt`/`build`/`vet`/`test`
（17 个包）与 `scripts/e2e/run.sh`（含浏览器检查）全绿。

执行中的两个实现要点（都是「三处一起改才成立」的耦合）：

1. **热切换的完整性**：`web.Frontend` 的词表受锁保护、`Sink` 持 `Frontend` 而不是词表快照、
   会话侧 `Emotions` 用取值函数——任一处留快照，就会出现「页面换了模型、会话还按旧词表摘标签」。
2. **推流的两种入口**：进程自动起与 `/api/stream` 启停共用 `streamRuntime.Start/Stop`；
   `Stop` 必须等关播收尾（进程先退出会把直播间挂成「直播中」，这是实测踩过的坑）。

遗留（未做，均为有意）：`go test -race` 未纳入门槛；`stream` 的 `exec.Command` 使用配置里的
二进制路径（semgrep 审计项，配置属运维可信托管）；`go-context-background-handler` 警告在
`streamStart`（推流要活过 HTTP 请求）与 `server.SendAction`（跨请求回写）处是有意为之。
