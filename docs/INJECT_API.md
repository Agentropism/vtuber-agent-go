...
**入队即返回**：响应 `200` 只表示已进入播报队列，不代表已经合成完、更不代表已经播完。播报的排队、抢占、冷却与并行合成统一由 `internal/agent/broadcast` 负责，因此事件回复与注入内容不会同时出声。
...
分级与实现位于 `internal/agent/broadcast`：`PriorityFor` 负责事件到优先级的映射，注入的取值在 `app` 装配处指定。

## 5. 表情的落地路径

`emotion` 随播报条目（`broadcast.Item.Emotion`）流经队列，由 `Sink` 在投递时按当前模型的 `emotionMap` 换算成 Live2D 表达式下标，经 `/client-ws` 的 `speak` 消息带给浏览器（见 `internal/agent/frontend`）。未配置 `[frontend]` 时退回只记日志的占位实现，表情不生效。
