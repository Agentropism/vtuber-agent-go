...
| 建立时机 | 应用启动时，`app.Initialize()` 调用 `upload.Init()` |
| 帧类型 | WebSocket 文本帧 |
| 连接超时 | 10 秒 |
| 压缩 | 禁用 WebSocket 压缩 |
| 初次连接失败重试 | 间隔 3 秒 |
...
调用上传函数后，事件只会被序列化并尝试放入本地队列，不会等待远端响应。队列已满时，网关记录错误日志并直接丢弃该事件。写入 WebSocket 失败时，该帧不会重新放回队列，连接随后重建。
...
记忆服务可在同一 WebSocket 连接中向网关发送 JSON 格式的 `action.Action`。网关会解析并记录 `Action` 字段，但当前没有执行、转发或确认回调动作的逻辑。因此，记忆服务不应依赖下行回调来驱动业务操作。
