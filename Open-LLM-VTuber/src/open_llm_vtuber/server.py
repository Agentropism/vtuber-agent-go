...
class WebSocketServer:
    ...
    (WebSocket, ASR, Live2D, proxy) and mounts static assets with CORS.
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        ws_handler = WebSocketHandler(default_context_cache=self.default_context_cache)
        ...
        self.app.include_router(
            init_asr_route(default_context_cache=self.default_context_cache),
        )
        self.app.include_router(init_live2d_model_route())

        # Initialize and include proxy routes if proxy is enabled
        system_config = config.system_config
        if hasattr(system_config, "enable_proxy") and system_config.enable_proxy:
            self.app.include_router(
                init_proxy_route(ws_handler=ws_handler),
            )
