    ...
    def create_agent(
        ...
        **kwargs,
        ...
        if conversation_agent_choice == "basic_memory_agent":
            ...
            # 本地长期记忆（可选，默认关闭）
            memory_store = None
            memory_settings: dict = basic_memory_settings.get("memory") or {}
            if memory_settings.get("enabled", False):
                from ..memory import MemoryStore

                memory_store = MemoryStore(
                    db_path=memory_settings.get("db_path", "memory.db"),
                    max_recall=memory_settings.get("max_recall", 5),
                    max_context_chars=memory_settings.get("max_context_chars", 1500),
                    min_importance=memory_settings.get("min_importance", 0.3),
                )
