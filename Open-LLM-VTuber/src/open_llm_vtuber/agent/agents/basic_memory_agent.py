...
from ...memory import MemoryStore, extract_rules
...
class BasicMemoryAgent(AgentInterface):
    ...
    def __init__(
        ...
        self._long_term_memories: List[Dict[str, Any]] = []
        self._memory_uid: str = ""
    ...
    def set_memory_from_history(self, conf_uid: str, history_uid: str) -> None:
        ...
        # 加载本地长期记忆（跨会话），并记录当前记忆归属身份
        self._memory_uid = conf_uid
        self._long_term_memories = []
        if self._memory_store:
            self._long_term_memories = self._memory_store.load_recent(
                conf_uid, self._memory_store.max_recall
            )
            logger.info(
                f"Loaded {len(self._long_term_memories)} long-term memories for {conf_uid}."
            )
    ...
    def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        messages = []
        if self._memory_store and self._long_term_memories:
            block = "\n".join(f"- {m['content']}" for m in self._long_term_memories)
            if block:
                max_chars = self._memory_store.max_context_chars
    ...
    def _store_memory(self, input_data: BatchInput) -> None:
        """按规则从本轮用户消息抽取事实并写入长期记忆。

        同步执行：规则匹配与 SQLite 写入均为微秒级，不阻塞回复流。
        """
        if not self._memory_store or not self._memory_uid:
            return
        text = self._to_text_prompt(input_data)
        if not text:
            return
        for content, kind, importance in extract_rules(text):
            if self._memory_store.add(self._memory_uid, content, kind, importance):
                if not any(m["content"] == content for m in self._long_term_memories):
                    self._long_term_memories.append(
