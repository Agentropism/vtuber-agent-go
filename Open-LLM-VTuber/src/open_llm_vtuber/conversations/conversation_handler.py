...
async def handle_conversation_trigger(
    ...
    if msg_type == "ai-speak-signal":
        # 若调用方显式提供了 user_input（如空闲自动发言），则直接使用；
        # 否则回退到配置的 proactive_speak_prompt 提示词。
        ...
        user_input = data.get("text", "")
        if data.get("upload_event"):
            metadata = {
                "upload_event": data["upload_event"],
                "upload_reply_client_id": data.get("upload_reply_client_id", ""),
            }
            upload_event = data["upload_event"]
            logger.info(
                "[upload:{}:{}:{}] dispatching queued message to conversation",
                upload_event.get("platform_name", "unknown"),
                upload_event.get("channel_id", "unknown"),
                upload_event.get("message_id") or upload_event.get("timestamp", "unknown"),
            )
    else:  # mic-audio-end
