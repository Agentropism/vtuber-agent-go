...
def init_client_ws_route(
    ...
    Args:
        ...
        ws_handler: Shared WebSocketHandler instance.
...
def init_proxy_route(ws_handler) -> APIRouter:
    """
    Create and return API routes for handling proxy connections.

    Args:
        ws_handler: WebSocketHandler instance for injecting messages

    Returns:
        APIRouter: Configured router with proxy WebSocket endpoint
    """
    router = APIRouter()
    proxy_handler = ProxyHandler(ws_handler)
...
def init_live2d_model_route() -> APIRouter:
    """创建并返回 Live2D 模型信息路由。"""
...
def init_asr_route(default_context_cache: ServiceContext) -> APIRouter:
    """创建并返回备用语音转写接口。"""
