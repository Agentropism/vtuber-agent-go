...
class BasicMemoryAgentConfig(I18nMixin, BaseModel):
    ...
    memory: Optional["MemoryConfig"] = Field(None, alias="memory")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "memory": Description(
            en="Local long-term memory configuration (default: disabled)",
            zh="本地长期记忆配置（默认关闭）",
        ),
    }


class MemoryConfig(I18nMixin, BaseModel):
    """本地长期记忆配置（默认关闭）。"""

    enabled: bool = Field(False, alias="enabled")
    db_path: str = Field("memory.db", alias="db_path")
    max_recall: int = Field(5, alias="max_recall")
    max_context_chars: int = Field(1500, alias="max_context_chars")
    min_importance: float = Field(0.3, alias="min_importance")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "enabled": Description(
            en="Enable local long-term memory (default: False)",
            zh="启用本地长期记忆（默认：False）",
        ),
        "db_path": Description(
            en="SQLite storage file path (default: memory.db, already gitignored)",
            zh="SQLite 存储文件路径（默认：memory.db，已在 .gitignore）",
        ),
        "max_recall": Description(
            en="Max memories injected per turn (default: 5)",
            zh="每轮注入的记忆条数上限（默认：5）",
        ),
        "max_context_chars": Description(
            en="Max characters of injected memory context (default: 1500)",
            zh="注入记忆上下文的字符上限（默认：1500）",
        ),
        "min_importance": Description(
            en="Memories below this importance are not stored/injected (default: 0.3)",
            zh="低于该重要度的记忆不存储/不注入（默认：0.3）",
        ),
    }
