...
from .proactive_speak import ProactiveSpeakConfig
...
class CharacterConfig(I18nMixin):
    ...
    proactive_speak_config: ProactiveSpeakConfig = Field(
        default_factory=ProactiveSpeakConfig, alias="proactive_speak_config"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "proactive_speak_config": Description(
            en="Configuration for idle proactive speak (automatically starting a topic when idle)",
            zh="空闲自动发言配置（空闲时自动发起话题）",
        ),
    }
