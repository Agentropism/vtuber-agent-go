...
- **开播被拒会自动重试**：开播阶段失败按退避重试（30s 起翻倍、5 分钟封顶）。B 站 的两类「要用户拿手机做一步」的拒绝被结构化处理（见 `stream.StartLiveError`）：`60024`（扫码验证）把验证地址出成二维码，`60043`（实名/人脸）给出带 `mid` 的认证页，两者都落到只允许本机访问的 `/login/verify`，页面每 3 秒自刷——用户扫完/刷完自动接上，不需要像参考项目那样「扫完手动再点一次开始」。`60045`（账号准入：注册>30天且粉丝>500）这类没有手机可做的动作，页面照实展示业务码与原话，服务继续按退避重试。
