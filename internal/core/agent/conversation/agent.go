...
func NewAgent(cfg AgentConfig) (*Agent, error) {
	...
	if cfg.MaxHistoryTurns > 0 {
		limits.MaxTurns = cfg.MaxHistoryTurns
	}
	if cfg.MaxHistoryTokens > 0 {
		limits.MaxTokens = cfg.MaxHistoryTokens
	}
...
func (a *Agent) Chat(ctx context.Context, userText string, onText func(string) error) error {
	return a.ChatWithContext(ctx, userText, "", onText)
}

// ChatWithContext 与 Chat 相同，但本轮额外带一段系统提示（例如召回的历史片段）。
//
// 单独开一个入口而不是把召回结果拼进用户消息：拼进去会写进历史，
// 后面每轮都会重复带上同一段旧记忆。
func (a *Agent) ChatWithContext(ctx context.Context, userText, extraSystem string, onText func(string) error) error {
	if onText == nil {
...
// trimHistory 按上限裁剪历史。
//
// 边写边裁而不是等发请求时再裁：这样长时间运行的进程不会把历史无限攒在内存里。
func (a *Agent) trimHistory() {
	a.history = TrimHistory(a.history, a.limits)
}
