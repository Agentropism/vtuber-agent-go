...
页面是自研的极简实现：`index.html` + `app.js` 原生 JS，无构建步骤，改完刷新即可。
`libs/` 里的三个文件随二进制一起嵌入（`//go:embed`），部署只需要一个可执行文件。
...
Live2D 的许可条款。它只有在渲染 Live2D 模型时才需要，`agent/frontend` 不依赖它。

## 为什么不直接用旧前端

原 `Open-LLM-VTuber/frontend/` 是一份 1.9 MB 的预构建 Vue 产物（子模块 build 分支），
需要配套实现 21 种下行消息、历史记录、配置切换等一整套协议才能跑起来。自研页面只认
本仓库需要的 2 种下行消息与 2 种上行回执，见 `agent/frontend/doc.go` 的协议说明。
