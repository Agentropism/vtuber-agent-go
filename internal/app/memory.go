...
func (m memoryAdapter) Remember(entry conversation.MemoryEntry) {
	if err := m.store.Append(memory.Entry{
		...
		m.log.Sugar().Warnf("写入记忆失败: %v", err)
	}
}

// provideMemory 打开长期记忆；未配置路径时返回 nil。
func provideMemory(cfg *config.Config, log *zap.Logger) (*memory.Store, error) {
	if cfg.Agent.MemoryFile == "" {
		log.Sugar().Info("未配置 [agent].memory_file，跳过长期记忆")
		return nil, nil
	}
	...
	log.Sugar().Infof("长期记忆已启用: %s（已有 %d 条记录）", cfg.Agent.MemoryFile, store.Len())
